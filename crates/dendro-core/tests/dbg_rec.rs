...
fn dbg_recovery() {
    ...
    let _ = std::fs::remove_dir_all(&dir);
    {
        ...
        for i in 0..100 { s.exec(&format!("INSERT INTO d VALUES ({i})")).unwrap(); }
    ...
    for (n, h) in &snap.manifest.refs {
        println!("ref {n}: wal_seg={} epoch={} covered={}", h.wal_seg, h.epoch, h.covered_seq);
    }
    let _ = std::fs::remove_dir_all(&dir);
}
