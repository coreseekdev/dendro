...
fn ap_materialize_and_scan() {
    ...
    s.exec("CREATE TABLE sales (id BIGINT PRIMARY KEY, region TEXT, amount DOUBLE)")
        .unwrap();
    ...
    let t0 = Instant::now();
    let out = s
        .exec("SELECT region, count(*), sum(amount) FROM sales GROUP BY region")
        .unwrap();
    ...
    s.exec("INSERT INTO sales VALUES (99999, 'north', 1.0)")
        .unwrap();

    // AP 路径聚合（物化后，行数 > 10k → 走 CBF + WAL overlay）
    let t1 = Instant::now();
    let out2 = s
        .exec("SELECT region, count(*) FROM sales GROUP BY region ORDER BY region")
        .unwrap();
