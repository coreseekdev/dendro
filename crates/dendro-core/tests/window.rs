...
fn setup(d: &Arc<Database>) {
    ...
    s.exec("CREATE TABLE w (id BIGINT PRIMARY KEY, grp TEXT, v INT)")
        .unwrap();
    s.exec("INSERT INTO w VALUES (1,'a',10),(2,'a',20),(3,'a',30),(4,'b',100),(5,'b',200)")
        .unwrap();
}
...
fn row_number_global() {
    ...
    let r = rows(
        &d,
        "SELECT row_number() OVER (ORDER BY v DESC) FROM w ORDER BY 1",
    );
    assert_eq!(r.len(), 5);
    assert_eq!(r[0][0], "1");
    assert_eq!(r[4][0], "5");
...
fn row_number_partition() {
    ...
    let r = rows(
        &d,
        "SELECT grp, row_number() OVER (PARTITION BY grp ORDER BY v) FROM w ORDER BY grp, v",
    );
...
fn rank_and_dense_rank() {
    ...
    let r = rows(
        &d,
        "SELECT rank() OVER (ORDER BY v), dense_rank() OVER (ORDER BY v) FROM w ORDER BY v",
    );
...
fn sum_over_partition() {
    ...
    let r = rows(
        &d,
        "SELECT grp, sum(v) OVER (PARTITION BY grp) FROM w ORDER BY grp, v",
    );
...
fn count_over_partition() {
    ...
    let r = rows(
        &d,
        "SELECT grp, count(*) OVER (PARTITION BY grp) FROM w ORDER BY grp, v",
    );
