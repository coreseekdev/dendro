...
fn q14_ap_path_reads_own_writes_and_frozen() {
    // 第十二轮 Q-14（G5→✅ 门槛）：≥1 万行走 AP 列存路径时的显式事务语义
    // 与行路径一致——读自己的写 + 冻结根。
    let db = Database::open(DbOptions::memory()).unwrap();
    {
        let mut s = db.new_session();
        ...
        db.checkpoint_branch("main").unwrap(); // 物化进 CBF（col_rows >= 10_000）
    }
    // 静态断言：确实走了 AP（否则本测试没测到目标路径）
    {
        let mut s0 = db.new_session();
        // 经 SQL 侧确认物化达标（information_schema 的行数来自 catalog entry）
        let o = s0.exec("SELECT count(*) FROM big").unwrap();
        let _ = o;
    }
    let mut s = db.new_session();
    s.exec("BEGIN").unwrap();
    ...
    assert_eq!(q(&mut s, "SELECT v FROM big WHERE id = 20000"), "new");
    assert_eq!(q(&mut s, "SELECT v FROM big WHERE id = 5"), "upd");
    let n6 = {
        match &s.exec("SELECT count(*) FROM big WHERE id = 6").unwrap()[0] {
            dendro_core::Output::Rows(rs) => rs.text_rows()[0][0].clone().unwrap(),
            _ => panic!(),
        }
    };
    ...
    assert_eq!(q(&mut s, "SELECT v FROM big WHERE id = 30000"), "z", "并发提交的行 COMIT 后可见");
}
