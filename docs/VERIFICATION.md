...
| 22 | JOIN ON 的 AND 链中非等值合取被**静默丢弃**（walk 返回值被忽略，scan.rs:1944-1953）→ `ON a.id=b.id AND a.v>1` 不过滤 v | 下推走查未回收残留条件 | 附录 A agent 实证（03a §A.4 Q4） | **未修**（P1）：残留合取须回填 Filter 或 0A000 拒绝；join 矩阵 slt 补非等值用例 |
| 23 | `count(*) WHERE <恒 NULL 谓词>` 无结果行（常量短路跳过聚合）vs `WHERE 1` 返回 count=0——两条路径不一致 | 常量谓词短路与聚合空集语义分叉 | 附录 A agent 实证（03a §A.4 Q5） | **未修**（P2）：定案"空输入全局聚合仍出一行"为唯一语义（对齐 02 §1 算子合同），常量短路路径修复 |
