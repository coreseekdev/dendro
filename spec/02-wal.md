...
1. **段是唯一写入单元**：8–64MB/段，PUT 一次成型；小事务靠组提交摊薄 RTT。
   **本地追加模式（P2-6e）**：`supports_append` 的存储（LocalObjStore）帧就地
   续写当前段、按 segment_bytes 封段（追加 32B trailer + 推进段号）——段对象数
   从"每 flush 一个"降为"每段字节阈值一个"；durable 提交消除建写+rename。
   追加非原子：崩溃留撕尾帧 → 恢复端对 **epoch 最后一段且未封段**（段尾无
   有效 trailer）的帧错误按撕裂写容忍；已封段帧损坏仍严格报错（真实腐坏）。
   append 失败 → 毒化且不回滚缓冲（部分字节可能在文件尾，回滚=重放重复帧），
   本批事务按 Uncertain 对账（§3.5，按段为不确定域）。
   **预分配 + 偏移写 + fdatasync（P2-6f）**：段首写前 fallocate 一个分块
   （min(segment_bytes, 4MB)，写满续配），帧按偏移覆写、fdatasync 只刷数据
   不刷元数据——durable 提交延迟 ~2×（实测 742→326µs p50，与 SQLite
   sync=FULL 持平）；封段缩回实际尺寸回收预留空间；崩溃打开段尾部为零页，
   由最后一段撕尾容忍吸收
...
- **唯一恢复路径 = reopen**（`engine.rs::reopen_branch` 或重启进程）：新 writer
  ...
  须自行对账（主键语义下重放同一事务为覆盖，天然幂等；非幂等写需业务键）。
- 毒化后**保活与惰性续期全部停止**（fence_gate 毒化快速失败）。注意：接管
  本不依赖旧租约过期（acquire 取 max epoch + 1）——停止续租的意义是
  **诚实**：不可用的写者不再占有一个"活着"的租约（审计/指标/运维视角）。
